/*
// Computes estimates for AR-MSIH models with EM-algorithm.
...
const double EPSILON_QUZHUO_BOUND = 0.001; // epsilon used for bounds as in Qu and Zhuo (2017)
...
Xi FilterARMSIHQuZhuo (arma::colvec* py,
                 arma::mat* py_lagged,
...
Xi ExpectationStepARMSIHQuZhuo	(arma::colvec* py,
                          arma::mat* py_lagged,
                          arma::mat* pz_dependent,
                          arma::mat* pz_independent,
                          Theta* ptheta)
{
  Xi filter = FilterARMSIHQuZhuo(py, py_lagged, pz_dependent, pz_independent,
                           ptheta);
...
Theta MaximizationStepARMSIHQuZhuo (arma::colvec* py,
                            arma::mat* py_lagged,
                            arma::mat* pz_dependent,
                            arma::mat* pz_independent,
                            ...
                            double sigma_min)
{
  ...
  arma::mat 		beta(s, 1, arma::fill::zeros); // s by M matrix WATCH: A case?
  ...
  // Impose Qu & Zhuo's constraint if needed
  if (arma::trace(transition_probs) < 1 + EPSILON_QUZHUO_BOUND)
  {
    double c = arma::trace(transition_probs) / (1 + EPSILON_QUZHUO_BOUND);
    for (int i = 0; i < M; i++)
      transition_probs(i,i) = transition_probs(i,i) / c;
  }
...
// [[Rcpp::export]]
SEXP EMcppARMSIHQuZhuo (Rcpp::NumericVector y_rcpp,
                  ...
                  double sigma_min)
{
  ...
  {
    index_exit++;
    Xi 		e_step = ExpectationStepARMSIHQuZhuo(&y, &y_lagged, &z_dependent, &z_independent,
                          &theta_updated);
    ...
    theta = theta_updated;
    theta_updated = MaximizationStepARMSIHQuZhuo(&y, &y_lagged,

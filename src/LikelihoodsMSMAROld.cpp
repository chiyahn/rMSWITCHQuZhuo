...
// [[Rcpp::export]]
SEXP LikelihoodsMSMAROld (Rcpp::NumericVector y_rcpp,
					...
					)
{
  ...
  arma::colvec likelihoods(n);
	...
	arma::mat** pz_dependent_lagged_blocks = new arma::mat*[s];
	arma::mat** pz_independent_lagged_blocks = new arma::mat*[s];
	for (int i = 0; i < s; i++)
	{
		pz_dependent_lagged_blocks[i] = new arma::mat(n, p, arma::fill::zeros);
		pz_independent_lagged_blocks[i] = new arma::mat(n, q, arma::fill::zeros);

		int z_dependent_block_first = i * p;
		int z_independent_block_first = i * q;
		*pz_dependent_lagged_blocks[i] = z_dependent_lagged.cols(z_dependent_block_first,
			z_dependent_block_first + p - 1);
		*pz_independent_lagged_blocks[i] = z_independent_lagged.cols(z_independent_block_first,
			z_independent_block_first + q - 1);
		pz_dependent_lagged_blocks[i]->row(190).print();
		pz_independent_lagged_blocks[i]->row(190).print();
	}
    ...
    xi_past /= arma::sum(xi_past);
		...
		for (int j_M = 0; j_M < M; j_M++)
		{
			for (int j_extra = 0; j_extra < M_extended_block; j_extra++)
			{
				...
				arma::colvec xi_k_t_jk = y.row(k) - mu(j_M);
		    ...
		    //   z_independent.row(k) * gamma_independent - mu(j_M);
				for (int lag = 0; lag < s; lag++)
				{
					...
					// xi_k_t_jk -= beta.at(lag, j_M) *
					// 							(y_lagged.at(k, lag) -
					// 							pz_dependent_lagged_blocks[lag]->row(k) * gamma_dependent.col(lagged_index) -
					// 							pz_independent_lagged_blocks[lag]->row(k) * gamma_independent -
					// 							mu(lagged_index));
				}
	...
	for (int i = 0; i < s; i++)
	{
		delete pz_dependent_lagged_blocks[i];
		delete pz_independent_lagged_blocks[i];
	}
	delete[] pz_dependent_lagged_blocks;
	delete[] pz_independent_lagged_blocks;
